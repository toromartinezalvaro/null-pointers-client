...
export default function Tarjetas() {
    ...
    const {indice, setIndice, respuestasSer, setRespuestasSer} = useDestino();
    ...
    const regresarPerfil = (destinoService: DestinoService) => {
        const confirmar = window.confirm("¿Desea crear un nuevo perfil y restablecer las opciones seleccionadas?");
        if (!confirmar) return;
        ...
        <main className="container__general">
            ...
            <div className="container">
                {opciones[indice].map((opcion, index) => (
                    <label key={index} htmlFor={`opc${index + 1}`}>{opcion}
                        <div className="card">
            ...
            <div className="navegacion">
                <ul>
                    <li>
                        <button className="perfil" onClick={() => {
                    ...
                    {contadores.map((contador, idx) => (
                        <li key={idx} className={contador}>
                            <button onClick={() => {setIndice(idx);setOpcSelect(""); }}>{idx + 1}</button>
                        </li>
                    ))}
                </ul>
            </div>
            <form className="botones">
                <button type="button" onClick={atras} disabled={disAtras}>Atrás</button>
                <button type="button" onClick={() => siguiente()} disabled={disSig} hidden={hidSig}>Siguiente</button>
                <Link to="/resultados">
                    <button type="button" hidden={calcular}>Calcular Destino</button>
                </Link>
            </form>
        </main>
    );
